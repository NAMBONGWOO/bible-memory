...
// [버그 수정] iOS 등 일부 환경에서 입력값이 한글 자소분리(NFD) 상태로 들어올 수 있음.
// 이 경우 시각적으로 동일한 글자("옮겼느니라")라도 문자 코드가 달라 매칭이 실패해
// 정확히 입력했는데도 오기입으로 잘못 판정되는 버그가 발생함 → NFC로 정규화해서 방지.
function toSyl(t) {
    return t.normalize('NFC').replace(/[.,·?!"'()\[\]]/g, '').replace(/\s+/g, '');
}
